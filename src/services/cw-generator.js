...
function trimTable(data) {
  ...
  for (let entry in words) {
    words[entry].length = words[entry].answer.length;
    words[entry].positions = [];

    if ("startx" in words[entry]) {
      words[entry].startx -= leftMost;
      words[entry].starty -= topMost;
      let x = words[entry].startx;
      let y = words[entry].starty;
      for (let char in words[entry].answer) {
        if (words[entry].orientation === "down") {
          words[entry].positions.push({
            x: x,
            y: y++,
            char: words[entry].answer[char],
          });
          continue;
        }
        words[entry].positions.push({
